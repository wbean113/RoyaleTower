    ...
    , cooldown(false)
{
    ...
    case CardType::CannonTower:
        m_pixmap.load(":/cards/cards/cannon_card.png");
        break;
    ...
    case CardType::Fireball:
        m_pixmap.load(":/cards/cards/fierball_card.png");
        break;
                     ...
                     QWidget *widget)
{
    ...
    if (m_flashError) {
        ...
        QPen flashPen(QColor(255, 50, 50, 220), 4);
        painter->setPen(flashPen);
        painter->drawRect(rect);
    }

    // 4. 冷却中时整体降低不透明度（文字标签也会受影响，因为它们作为子item独立绘制）
    if (cooldown) {
        setOpacity(0.5);
    } else {
        setOpacity(1.0);
